...
public class XaDataSource extends AbstractDataSource implements XaStateChangeListener {
	...
	public void prepare(XaMessage event) throws Exception {
		if (cache.get(event.getXaId()).isClosed()) {
	...
	@Override
	public void commit(XaMessage event) throws Exception {
		cache.get(event.getXaId()).commit();
		if (log.isTraceEnabled()) {
	...
	@Override
	public void rollback(XaMessage event) throws Exception {
		cache.get(event.getXaId()).rollback();
		if (log.isTraceEnabled()) {
	...
	@Override
	public void rollback(XaMessage event, Exception e) throws Exception {
